      ...
      <div className="flex-1 flex flex-col items-center justify-start ml:[28] xl:ml-[60px] 2xl:ml-[180px] px-4 pt-28 md:pt-6 pb-6 FontGeologica relative w-full overflow-y-auto">
        <div className="mt-30 sm:mt-15 xl:ml-70 2xl:ml-100 bg-[var(--Voscuro2)] p-6 rounded-lg w-full max-w-[800px] max-h-[70vh] overflow-y-auto overflow-x-hidden">
          ...
          <div className="flex flex-col md:flex-row gap-8 mb-6">
            ...
            <input type="text" className="text-white rounded-md border border-[var(--Vclaro3)] text-center w-full h-12 md:w-120 text-sm md:text-xl" placeholder="Buscar camión..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />
          </div>
          ...
          <div className="w-full overflow-x-auto text-white">
            ...
            {camionesFiltrados.map((camion, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-7 gap-3 md:gap-2 text-left md:text-center text-sm md:text-lg p-4 border border-[var(--Vclaro3)] min-w-[700px]">
                <p>{camion.placa}</p>
                <p>{camion.modelo}</p>
                ...
                <p>{camion.tipo_c}</p>
                <p>{camion.marca}</p>
                <div className="flex gap-2 md:justify-center justify-start">
                  <button onClick={() => { setShowForm(true); setModoEdicion(true); setCamionEditarIndex(index); setNuevoCamion(camion); }} className="w-10 h-10 bg-[var(--Vclaro3)] text-white rounded-md flex items-center justify-center hover:scale-105"><MdEdit /></button>
                  <button onClick={() => handleEliminar(index)} className="w-10 h-10 bg-[var(--Rojo)] text-white rounded-md flex items-center justify-center hover:scale-105"><AiOutlineDelete /></button>
                </div>
              </div>
            ))}
          </div>
