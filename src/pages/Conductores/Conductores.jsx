...
const Conductores = () => {
  ...
  const handleSubmitDriver = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:10101/agregarConductor/conductores', {
        id_rol,
        email,
        nombres,
        apellidos,
        ...
        <div className="mt-35 sm:mt-10 2xl:ml-80 xl:ml-50 bg-[var(--Voscuro2)] p-6 rounded-lg w-full max-w-[900px]">
          ...
          <div className="w-full text-white">
            ...
            {conductoresFiltrados.length === 0 ? (
              ...
              <div 
                key={index} 
                className={`border border-[var(--Vclaro3)] rounded-md p-4 mb-4 text-white
                  md:grid md:grid-cols-7 md:text-center md:p-4 md:mb-0 md:items-center
                `}
              >
                {/* Móvil: etiquetas visibles, layout vertical */}
                <div className="block md:hidden mb-2">
                  ...
                  <p><span className="font-bold">Email: </span><span className="truncate max-w-[150px]" title={conductor.email}>{conductor.email}</span></p>
